...
void i2c_master_Init(void)
{
  ...
  dev_cfg.device_address = 0x18; // ES8311 codec
...
uint8_t i2c_master_touch_write(i2c_master_dev_handle_t dev_handle,uint8_t *buf,uint8_t len)
{
  uint8_t ret;
  ret = i2c_master_bus_wait_all_done(user_i2c_port1_handle,i2c_done_pdMS_TICKS);
  if(ret != ESP_OK)
...
uint8_t i2c_master_touch_read(i2c_master_dev_handle_t dev_handle,uint8_t *buf,uint8_t len)
{
  uint8_t ret;
  ret = i2c_master_bus_wait_all_done(user_i2c_port1_handle,i2c_done_pdMS_TICKS);
  if(ret != ESP_OK)
...
uint8_t es8311_write_reg(uint8_t reg,uint8_t val)
{
  uint8_t buf[2] = {reg, val};
  uint8_t ret = i2c_master_bus_wait_all_done(user_i2c_port0_handle,i2c_done_pdMS_TICKS);
  if(ret != ESP_OK)
  return ret;
  ret = i2c_master_transmit(es8311_dev_handle,buf,2,i2c_data_pdMS_TICKS);
  return ret;
}

uint8_t es8311_read_reg(uint8_t reg,uint8_t *val)
{
  uint8_t ret = i2c_master_bus_wait_all_done(user_i2c_port0_handle,i2c_done_pdMS_TICKS);
  if(ret != ESP_OK)
  return ret;
  ret = i2c_master_transmit_receive(es8311_dev_handle,&reg,1,val,1,i2c_data_pdMS_TICKS);
  return ret;
}
